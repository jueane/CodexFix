...
```powershell
$files = @('.\New-CodexPatchedCopy.ps1')
...
测试时可用 `New-CodexPatchedCopy.ps1 -SourcePackageDir <测试包目录> -PortablePackageDir <临时目标目录>` 检查修补结果、备份及重复运行的幂等性。测试包必须包含 `app\ChatGPT.exe` 和 `app\resources\app.asar`；操作前关闭 Codex。
...
`New-CodexPatchedCopy.ps1` 内的 `Update-CodexAsar` 是 portable 副本的字节级修补函数。它读取副本的 `app.asar`，并应用两个只针对当前最新 Codex 版本的补丁：
...
修补函数会拒绝模糊或不支持的输入：每个补丁必须精确匹配一个原始片段，或者已经存在一个已修补片段。写入后它会验证原始片段已消失，且每个补丁恰好存在一个已修补片段。

`New-CodexPatchedCopy.ps1` 是这台机器上的首选流程。它会在 `C:\Program Files\WindowsApps` 下找到最新的 `OpenAI.Codex_*_x64__2p2nqsd0c76g0` 安装包，用 `robocopy` 复制完整包到 `portable\`，修补副本中的 `app\resources\app.asar`，并刷新桌面与仓库根目录的原版快捷方式。这样可以避免修改 WindowsApps。

双击 `Start-CodexPatched.cmd` 启动补丁版；它调用同目录的 `Start-CodexPatched.ps1`，后者从同目录的 `portable\` 中选版本号最新的 `app\ChatGPT.exe`，通过 PowerShell 的 `Invoke-CommandInDesktopPackage` 借用已安装 Codex 的 Windows 包身份启动。不生成补丁版快捷方式。修补逻辑直接在创建副本脚本中。
...
跟踪源码应保持为 PowerShell 脚本、`Start-CodexPatched.cmd`、`README.md`、`CLAUDE.md`、`.gitignore`，以及少量元数据，例如 `test/original.sha256.txt`。
...
Codex 更新后，先检查新版 `app.asar` 中更新后的 `/wham/tasks/list` 和 `/wham/usage` 片段，把 `New-CodexPatchedCopy.ps1` 中的补丁片段改为只支持该最新版本并移除旧版匹配，然后重新运行 `New-CodexPatchedCopy.ps1`。
